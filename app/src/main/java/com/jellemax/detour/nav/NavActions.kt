 ...
 *
 * These are extension functions on `MutableList<Destination>` rather than
 * methods on Nav 3's `NavBackStack`, and that is the point: `NavBackStack` *is* a
 * `MutableList`, so the app calls these on the real stack while a test calls them
 * on an `ArrayList`. Every edge this app can take becomes assertable under
 ...
 */
fun MutableList<Destination>.push(to: Destination) {
 ...
 */
fun MutableList<Destination>.pop() {
 ...
 */
fun MutableList<Destination>.returnToMap() {
