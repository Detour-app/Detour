...
| **State** | **in progress** — machines 1 and 2 landed 2026-08-13. `SectionAverageTracker` (`39da928`, `0903d14`) and `CameraWarner` (`dedba5a`, `d49d91e`, `bde8be3`) are in `shared/…/drive/`; 152 unit tests, 0 failures. **Machine 3 (`SpeedLimitTracker`) is held** — `CameraWarner.onFix` takes `limitKmh`, which is machine 3's own output, so a chime regression would be attributable to either machine; see the Deviation section in [`../DECISION.md`](../DECISION.md). **Both replay gates are outstanding** and need one healthy Overpass mirror (`.claude/skills/detour-gps-replay/scripts/overpass-ready.sh`); one `trajectcontrole.txt` run closes both, since the chip and the chime come off the same route. Note the preconditions below now fail *because this stage is underway* — `var active: SpeedCameras.Section` is 0 precisely because machine 1 removed it. That is the stage succeeding, the same inversion stage 0's preconditions carry. |
