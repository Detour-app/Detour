...
| **State** | **done** 2026-08-13 — all three machines in `shared/…/drive/` across nine commits (`39da928`…`97379ba`): `SectionAverageTracker` (15 tests), `CameraWarner` (11), `SpeedLimitTracker` (14). 166 unit tests, 0 failures; shared-core gate 7/7; Kotlin/Native verified in CI. `car/` no longer carries its own camera-warn or speed-limit copy. `MapScreen.kt` 1701 → **1640** lines — above the plan's revised 1500–1560 target, stated rather than massaged. **No GPS replay was run for any machine**: both app mirrors refuse this host and the one healthy public mirror is a Switzerland-only extract, so the three replay gates are outstanding by an explicit maintainer decision to trade behaviour verification for completion. What that leaves unverified is recorded in each machine's report and in the PR description. |
