...
| `Camera:BaseUrl` | Base URL that serves `/api/cameras` — typically your own API's public URL, since cameras are served by Detour.Api itself. Optional — blank means not announced, and the app falls back to querying the public Overpass API. Also stated on `/api/capabilities` (issue #303), same reasoning as `Idp:Authority` above. Load data with `dotnet Detour.Api.dll import-cameras <path.json>`, which upserts an importer JSON file — see `tools/camera-importer/osm_import.py` (OSM) and `tools/lufop-importer/lufop_import.py` (LUFOP) for what produces one. |
| `SpeedLimit:BaseUrl` | Base URL that serves `/api/speedlimits` — typically your own API's public URL, same as `Camera:BaseUrl` above. Optional — blank means not announced, and the app falls back to a disk cache, then to the public Overpass API. Also stated on `/api/capabilities` (issue #379). Load data with `dotnet Detour.Api.dll import-speedlimits <path.json>`, which upserts an importer JSON file — see `tools/speedlimit-importer/osm_import.py`. |
| `Municipality:BaseUrl` | Base URL that serves `/api/municipality` — same as `Camera:BaseUrl` above. Optional — blank means not announced, and the app falls back to the public Overpass API. Also stated on `/api/capabilities` (issue #381). Load data with `dotnet Detour.Api.dll import-municipalities <path.json>` — see `tools/municipality-importer/osm_import.py`. |
| `Poi:BaseUrl` | Base URL that serves `/api/pois` — same as `Camera:BaseUrl` above. Optional — blank means not announced, and the app falls back to the public Overpass API. Also stated on `/api/capabilities` (issue #383). Load data with `dotnet Detour.Api.dll import-pois <path.json>` — see `tools/poi-importer/osm_import.py`. |
