...
if (args.Length == 2 && args[0] == CameraImport.Command)
{
    await CameraImport.RunAsync(args[1], app.Services);
    return;
}

if (args.Length == 2 && args[0] == SpeedLimitImport.Command)
{
    await SpeedLimitImport.RunAsync(args[1], app.Services);
    return;
}

if (args.Length == 2 && args[0] == RoadImport.Command)
{
    await RoadImport.RunAsync(args[1], app.Services);
    return;
}
...
if (args.Length == 2 && args[0] == PoiImport.Command)
{
    await PoiImport.RunAsync(args[1], app.Services);
    return;
}
