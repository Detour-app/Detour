...
public static class CameraImport
{
    ...
    public static async Task RunAsync(string jsonPath, IServiceProvider services)
    {
        ...
        foreach (var el in doc.RootElement.GetProperty("cameras").EnumerateArray())
        {
            ...
            {
                // Structurally malformed entry (missing property, unrecognized `kind`, non-numeric
                // lat/lon, …) — count it and move on rather than aborting the whole run: with
                // hundreds/thousands of OSM-derived entries per region (Task 8), one bad row must
                // not discard every entry already processed (all DB writes are deferred to the
                // single FlushChangesAsync below, so an uncaught throw here would have flushed
                // nothing at all, not just skipped this entry).
                skipped++;
