...
def _write_fixture(path):
    ...
    # A drivable way with a maxspeed tag — kept, unlike the speedlimit importer this table
    # doesn't care whether one is present.
    ...
    w.add_way(osmium.osm.mutable.Way(id=20, nodes=[4, 5], tags={"highway": "footway"}))

    # Drivable and untagged — kept, unlike the speedlimit importer which would drop it.
    ...
    # A single-node way — no geometry to snap against, excluded.
    w.add_node(osmium.osm.mutable.Node(id=8, location=(7.000, 53.000)))
    w.add_way(osmium.osm.mutable.Way(id=40, nodes=[8], tags={"highway": "tertiary"}))
...
def main():
    ...
    assert set(ways) == {"w10", "w30"}, f"expected only the drivable multi-point ways, got {set(ways)}"

    tagged = ways["w10"]
    assert tagged["highway"] == "primary"
    ...
    untagged = ways["w30"]
    assert untagged["highway"] == "residential", "untagged (no maxspeed) drivable ways must still be kept"

    print(f"OK: {len(result['ways'])} ways, non-drivable and single-point ways excluded")
